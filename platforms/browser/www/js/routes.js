var routes = [
  ...
  // About page
 {
  ...
  // Right Panel pages
  {
    ...
    {
    path: '/newpassword/',
    url: './pages/newpassword.html',
  },
  {
    path: '/dttpage/',
    url: './pages/dttpage.html',
    on: {
        pageBeforeIn: function (event, page) {
          // do something before page gets into the view
          if(!localStorage.CardId) {
            localStorage.CardId = "5555555555555550"
          }
        },
        pageAfterIn: function (event, page) {
              ...
              $$("#donglelist").html("");
              app.methods.accountinfo(JSON.stringify(param), function(res) {
                  var obj = JSON.parse(res);
                  console.log(res);
                  Customer = obj.PowerSearchResult.Customer;
                  CustomerDetail = obj.PowerSearchResult.CustomerDetail;
                  CustomerAddress = obj.PowerSearchResult.CustomerAddress;
                  console.log(Customer);
                  if(obj.PowerSearchResult.Packages) {
                       for(var m=0; m<obj.PowerSearchResult.Packages.length;m++) {
                            var html ='<li><div class="item-content"><div class="item-inner">';
                                 html =  html + '<div class="item-title">'+obj.PowerSearchResult.Packages[m].CardId+'<br /><small>'+obj.PowerSearchResult.Packages[m].PackageExtDesc+' <br />(Subscription Until: '+moment(obj.PowerSearchResult.Account.CdSubscriptionUntil).format('MM/DD/YYYY')+')</small></div>';
                                   html =  html + '<div class="item-after">PHP '+obj.PowerSearchResult.Packages[m].Price+'</div>';
                                  html =  html + '</div></div></li>';
                              $$("#donglelist").append(html);
                        }
                  }
              });
        },
  ...
  },
   {
    ...
    on: {
        pageBeforeIn: function (event, page) {
          ...
          app.methods.getpaymenttoken(Card, function(res) {
              var pobj = JSON.parse(JSON.parse(res));
              console.log(pobj);
              if(pobj.state == "AVAILABLE") {
                PaymentToken = pobj;
              } else {
                  alert("Vaildate your card information");
              }
           });
        },
        pageAfterIn: function (event, page) {
            $$(".plan_name").html(Checkout.plan.name);
            $$(".plan_price").html(Checkout.plan.currency + " " + Checkout.plan.amount);
            $$(".plan_details").html(Checkout.plan.description);
            $$("#firstname").val(Customer.Firstname);
            $$("#lastname").val(Customer.Lastname);
            $$("#street").val(CustomerAddress.HouseNoStreetName);
        },
        pageInit: function (event, page) {
          // do something when page initialized
        },
     ...
     on: {
        ...
        pageAfterIn: function (event, page) {
            ...
            for(var m=0; m<Month.length;m++) {
              $$("#expMonth").append('<option class="'+Month[m]+'">'+Month[m]+'</option>');
            }
        },
        pageInit: function (event, page) {
          // do something when page initialized
